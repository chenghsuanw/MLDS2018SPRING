...
def main():
	...
	with tf.variable_scope('Hidden_Layers'):
		...
		W2 = tf.get_variable('W2', initializer=tf.random_normal([128, 247], stddev=0.1))
		b2 = tf.get_variable('b2', initializer=tf.random_normal([247], stddev=0.1))
		layer2 = tf.nn.leaky_relu(tf.matmul(layer1, W2) + b2)

		W3 = tf.get_variable('W3', initializer=tf.random_normal([247, 10], stddev=0.1))
		b3 = tf.get_variable('b3', initializer=tf.random_normal([10], stddev=0.1))
		predict = tf.nn.leaky_relu(tf.matmul(layer2, W3) + b3)
	...
	with open('loss_DNN3.pickle', 'wb') as f:
		pickle.dump(loss_train_list, f)
	with open('acc_DNN3.pickle', 'wb') as f:
