...
tf.app.flags.DEFINE_string('model_path', default='./model/basic_model', help='model path')
...
def main():
	...
	training_data = load_training_data(FLAGS.train_label)

	if not os.path.exists('w2i.pickle'):
		build_dictionary(training_data[:,1])
	with open('w2i.pickle', 'rb') as f:
		w2i = pickle.load(f)

	if not os.path.exists('w2v.pickle'):
		build_word_vector(w2i, FLAGS.word2vec, FLAGS.word_embed_dim)
	with open('w2v.pickle', 'rb') as f:
		w2v = pickle.load(f)

	data = Data()
	data.tokenize(data=training_data, w2i=w2i, max_length=FLAGS.decoder_length)

	if FLAGS.model == 'basic':
		model = CaptionGeneratorBasic(FLAGS, w2v)
	else:
		model = CaptionGeneratorAttention(FLAGS, w2v)
	
	model.train(data)



	IDs, testing_data = load_testing_data(FLAGS.test_file)
